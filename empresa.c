...
TipoRet CrearOrg(Empresa &e, Cadena cargo){
	...
	// Verifica si la empresa ya tiene un organigrama
    if (e.raiz != nullptr) {
        return ERROR; // Ya existe un organigrama
    }

    // Crea el nodo raíz
    e.raiz = new nodo_empresa(cargo);
    
    // Retorna OK si se creó con éxito
    return OK; 
}
...
TipoRet EliminarOrg(Empresa &e){
	...
	// Verifica si la empresa tiene un organigrama
    if (e != nullptr) {
        // Recorre y libera cada subcargo
        for (auto subcargo : e->subcargos) {
            // Elimina recursivamente los subcargos
            EliminarOrg(subcargo);
        }

        // Luego libera el nodo actual
        delete e; // Libera la memoria del nodo actual
        e = nullptr; // Asigna nullptr a la empresa para indicar que está vacía
    }
    
    return OK; // Retorna OK indicando que se eliminó con éxito
}
